...
const pokedexSpeak = (name, type, species, mainAttack, secondaryAttack, detail, detail2, detail3) => {
	const phrase = `
...
const renderError = (err) => {
	...
	$secondScreen.innerHTML = '<p>No encontramos lo que buscabas, intentalo nuevamente...</p>';
	$searchInput.removeAttribute('disabled');
	$btnStartSearch.classList.remove('disabled');
};

//!!!!!!!Llamada a la API de Pokémon!!!!!!!! ALL POKEMON ARE COMING IN
async function getDataPokemon() {
	let searchValue = $searchInput.value.toLowerCase();
	let pokeApi = `https://pokeapi.co/api/v2/pokemon/${searchValue}`;

	try {
		$secondScreen.innerHTML = `<img class="loader" src="./Assets/Loaders/svg-loaders/puff.svg"  alt="Cargando...">`;
		$mainScreen.innerHTML = `<img class="loader" src="./Assets/Loaders/svg-loaders/puff.svg"  alt="Cargando...">`;
      ...
      `;
			...
			const detailsPokemon = pokemonDetails.filter((detail) => detail.language.name === 'es'); //THE FUCK??????
...
d.addEventListener('click', (e) => {
	...
	if (!$powerBtn.classList.contains('is-off')) {
		if (e.target === $btnStartSearch) {
			$searchInput.value !== '' ? getDataPokemon() : alert('Debe ingresar el numero o nombre del pokémon que desea buscar');
		}
	...
	if (e.target.matches('.buttons-circle') || e.target.matches('.buttons-circle img')) {
		$soundPowerBtn.src = './Assets/016 Pokémon Healed.mp3';
		$soundPowerBtn.play();
